...
public class SerializationClientApplication extends Application<SerializationClientConfiguration> {
	...
	public void run(final SerializationClientConfiguration configuration, final Environment environment) {
		...
		environment.jersey().register(new SequenceResource(client, configuration.getApplicationServerConfiguration()));
		environment.healthChecks().register(configuration.getClientName(), new ConnectionHealthCheck(client, configuration.getApplicationServerConfiguration()));
